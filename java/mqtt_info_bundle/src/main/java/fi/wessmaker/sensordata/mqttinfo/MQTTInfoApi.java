...
public class MQTTInfoApi implements MQTTInfoService {
	...
	public Response getAllTopics () {
		return Response.status(Status.OK).entity(MQTTHandler.get().getMQTTInfo().getCustomTopics())
				.build();
	}
	...
	public Response getTopicCount () {
		return Response.status(Status.OK)
				.entity(MQTTHandler.get().getMQTTInfo().getCustomTopics().size()).build();
	}
	
	@Override
	public Response getConnection () {
		return Response.status(Status.OK).entity(MQTTHandler.get().getMQTTInfo().isConnection())
				.build();
	}
}
